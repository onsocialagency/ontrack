...
/**
 * Condensed "top suggestions" widget shown on overview pages.
 * Collapsed by default on all screen sizes. When expanded, shows every
 * suggestion with dividers between rows and scrolls once the list is tall.
 */
export function SuggestionWidget() {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4 sm:p-5 overflow-hidden">
      {/* Header — acts as the collapse toggle on all breakpoints */}
      <div className="flex items-center justify-between gap-3">
        ...
        >
          ...
          <ChevronDown
            size={16}
            className={cn(
              "text-[#8192A6] flex-shrink-0 transition-transform",
              open && "rotate-180",
            )}
          />
        </button>
      ...
      {/* Collapsible body */}
      {open && (
        <div id="suggestion-widget-body" className="mt-3">
          {/* Empty state */}
          ...
          {/* Full list with dividers; scrolls when > 3 items */}
          {items.length > 0 && (
            <div
              className={cn(
                "rounded-lg border border-white/[0.04] divide-y divide-white/[0.06]",
                items.length > 3 && "max-h-[264px] overflow-y-auto",
              )}
            >
              {items.map((s) => (
                <Link
                  key={s.id}
                  href={fullHref}
                  className="flex items-start gap-2.5 px-3 py-2.5 hover:bg-white/[0.04] transition-colors group"
                >
                  <span
                    className={cn(
